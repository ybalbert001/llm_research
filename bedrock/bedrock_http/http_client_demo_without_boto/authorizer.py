...
def authorize(payload, payload_hash=None, stream=False):
    ...
    if stream:
        canonical_uri = get_canonical_url(conf.BEDROCK_ENDPOINT_STREAM_URL)
    else:
        canonical_uri = get_canonical_url(conf.BEDROCK_ENDPOINT_URL)
